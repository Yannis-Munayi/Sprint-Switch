...
func _process(delta):
		if Input.is_action_just_pressed("switch_to_1"):
			if player_2.visible == true: # Ensures that player 2's "movement" is carried over to player 1
			...
			player_2.position = player_3.position
			...
			if player_2.visible == true: # Ensures that player 2's "movement" is carried over to player 3
				player_1.position = player_2.position
				player_3.position = player_2.position
